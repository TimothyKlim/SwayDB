...
sealed trait SegmentReadSpec extends TestBase with ScalaFutures with PrivateMethodTester {
  ...
  "Segment.belongsTo" should {
    "return true if the input key-value belong to the Segment else false when the Segment contains no Range key-value" in {
      ...
      Segment.belongsTo(Transient.remove(6), segment) shouldBe false
    ...
    "return true if the input key-value belong to the Segment else false when the Segment's max key is a Range key-value" in {
      ...
      Segment.belongsTo(Transient.remove(7), segment) shouldBe true
      Segment.belongsTo(Transient.remove(10), segment) shouldBe false
    ...
    "return true if the input key-value belong to the Segment else false when the Segment's min key is a Range key-value" in {
      ...
      Segment.belongsTo(Transient.remove(6), segment) shouldBe true
      Segment.belongsTo(Transient.remove(7), segment) shouldBe true
      Segment.belongsTo(Transient.remove(10), segment) shouldBe true
  ...
  "Segment.overlaps" should {
    "return true for overlapping Segments else false for Segments without Ranges" in {
      ...
      var segment2 = TestSegment(Slice(Transient.remove(2), Transient.put(3)).updateStats).assertGet
      ...
      segment2 = TestSegment(Slice(Transient.remove(2), Transient.put(3)).updateStats).assertGet
  ...
  "Segment.getAllKeyValues" should {
    "get KeyValues from multiple Segments" in {
      val keyValues1 = randomKeyValues(keyValuesCount, addRandomRemoves = true)
      val keyValues2 = randomKeyValues(keyValuesCount, addRandomRemoves = true)
      val keyValues3 = randomKeyValues(keyValuesCount, addRandomRemoves = true)
  ...
  "Segment.getAll" should {
    "read full index" in {
      val keyValues = randomKeyValues(keyValuesCount, addRandomRemoves = true, addRandomRanges = true)
  ...
  "Segment.getNearestDeadlineSegment" should {
    "return None deadline if non of the key-values in the Segments contains deadline" in {
      ...
      val segment1 = TestSegment(randomKeyValues(100)).assertGet
      val segment2 = TestSegment(randomKeyValues(100)).assertGet
