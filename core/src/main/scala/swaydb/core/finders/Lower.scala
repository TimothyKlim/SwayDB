                        ...
                        next match {
                          case Some(next) =>
                              ...
                              next.toValue() match {
                                case Success(nextValue) =>
                                  KeyValueMerger(Some(next.key), rangeValue, nextValue: Value.FromValue) match {
                                    case Success(current) =>
                                      Max(current.toMemory(next.key), None) match {
                                        case found @ Success(Some(_)) =>
                                          found
                            ...
                            //10
                              ...
                              next.toValue() match {
                                case Success(nexrtValue) =>
                                  KeyValueMerger(Some(next.key), fromValue.getOrElse(rangeValue), nexrtValue: Value.FromValue) match {
                                    case Success(current) =>
                                      Max(current.toMemory(next.key), None) match {
                                        case found @ Success(Some(_)) =>
                                          found
                              ...
                              next match {
                                case Some(next) =>
                                  ...
                                  if (current.key equiv next.key)
                                    next.toValue() match {
                                      case Success(nextValue) =>
                                        KeyValueMerger(Some(next.key), fromValue, nextValue: Value.FromValue) match {
                                          case Success(current) =>
                                            Max(current.toMemory(next.key), None) match {
                                              case found @ Success(Some(_)) =>
                                                found
                            ...
                            next.toValue() match {
                              case Success(nextValue) =>
                                KeyValueMerger(Some(current.fromKey), fromOrElseRangeValue, nextValue: Value.FromValue) match {
                                  case Success(merged) =>
                                    Max(merged.toMemory(current.fromKey), None) match {
                                      case found @ Success(Some(_)) =>
                                        found
