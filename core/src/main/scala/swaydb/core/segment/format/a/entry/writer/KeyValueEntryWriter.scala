...
import swaydb.core.segment.format.a.entry.id.KeyValueToEntryIdBinding
...
object KeyValueEntryWriter {
  ...
  def write[T <: KeyValue.WriteOnly](current: T,
                                     ...
                                     compressDuplicateValues: Boolean)(implicit bind: KeyValueToEntryIdBinding[T]): (Slice[Byte], Option[Slice[Byte]], Int, Int) = {
    current.previous flatMap {
          ...
          case (_, remainingBytes) if remainingBytes.isEmpty =>
            ...
            val (indexBytes, valueBytes, valueStartOffset, valueEndOffset) =
              ValueWriter.write(
                ...
                entryId = bind.entryId.format.keyFullyCompressed,
                appliedFunctions = applies,
          ...
          case (commonBytes, remainingBytes) =>
            val (indexBytes, valueBytes, valueStartOffset, valueEndOffset) =
              ValueWriter.write(
                current = current,
                compressDuplicateValues = compressDuplicateValues,
                entryId = bind.entryId.format.keyPartiallyCompressed,
                appliedFunctions = applies,
      ...
      val (indexBytes, valueBytes, valueStartOffset, valueEndOffset) =
        ValueWriter.write(
          current = current,
          compressDuplicateValues = compressDuplicateValues,
          entryId = bind.entryId.format.keyUncompressed,
          appliedFunctions = applies,
